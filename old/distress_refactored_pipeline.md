...
CONFIG = {
    ...
    "COST_FN": 10.0,
    "COST_FP": 1.0,
    "CAPACITY_PCT": 0.10,  # screen top 10% by PD as a capacity policy
...
```
    ...
    Created missingness flags for: ['at', 'dlc', 'dltt', 'seq', 'mibt', 'oibdp', 'oancf', 'xint', 'act', 'lct', 'che', 'rect', 'invt', 'mkvalt', 'dv', 'dvc', 'dvt', 'dvp']
...
def peer_median_impute(df_in: pd.DataFrame, cols: list[str], train_mask: pd.Series, group_cols: list[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Impute NaNs using TRAIN-only medians by group_cols, with TRAIN global median fallback."""
    df_out = df_in.copy()
...
```python
audit_cols = [c for c in ["at","dlc","dltt","seq","oibdp","oancf","mkvalt","act","lct"] if c in df.columns]
...
```python
eda_cols = [c for c in ["at","mkvalt","dlc","dltt","seq","oibdp","oancf","xint"] if c in df.columns]
...
plot_cols = [c for c in ["at","mkvalt","dltt","dlc","oibdp","oancf"] if c in df.columns]

for c in plot_cols[:4]:
...
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_33_0.png)
    



    
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_33_1.png)
    



    
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_33_2.png)
    



    
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_33_3.png)
...
df["log_mkvalt"] = np.log(pd.to_numeric(df["mkvalt"], errors="coerce").where(lambda s: s > 0)) if "mkvalt" in df.columns else np.nan

# Basic sanity assertions (soft; may fail in synthetic or messy real data)
if not USING_SYNTHETIC_DATA:
    assert (pd.to_numeric(df["at"], errors="coerce") > 0).mean() > 0.95, "Many non-positive assets; check data."
```
...
```
    ...
    Continuous (to be scaled): ['sp_cfo_to_debt', 'sp_focf_to_debt', 'sp_dcf_to_debt', 'log_at', 'log_mkvalt']
    Events (kept in levels): ['evt_divcut', 'evt_divsusp', 'evt_divinit', 'evt_liq_squeeze', 'evt_quick_squeeze', 'evt_ebitdadrop', 'evt_cfdrop']
    MODEL_FEATS (post-scaling names): ['z_sp_cfo_to_debt', 'z_sp_focf_to_debt', 'z_sp_dcf_to_debt', 'z_log_at', 'z_log_mkvalt', 'evt_divcut', 'evt_divsusp', 'evt_divinit', 'evt_liq_squeeze', 'evt_quick_squeeze', 'evt_ebitdadrop', 'evt_cfdrop']
...
```
    ...
    {'train': (44783, 12), 'val': (6415, 12), 'test': (12404, 12)}
...
```
    ...
    Marginal effects computation failed: Length mismatch: Expected axis has 12 elements, new values have 13 elements
...
### 7B. Tree-based model (XGBoost; nonlinear benchmark)
...
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_77_2.png)
...
```
    ...
    val | logit: calibration intercept=0.141, slope=0.984



    
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_81_1.png)
...
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_81_3.png)
    


    test | logit: calibration intercept=0.104, slope=0.998



    
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_81_5.png)
    


    test | tree_calibrated: calibration intercept=-0.182, slope=0.922



    
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_81_7.png)
...
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_83_1.png)
...
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_89_0.png)
    



    
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_89_1.png)
...
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_97_0.png)
    


    logit cost-opt thr= 0.05 capacity thr= 0.3814677864689543



    
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_97_2.png)
    


    tree_calibrated cost-opt thr= 0.09 capacity thr= 0.5288888812065125
...
![png](distress_refactored_pipeline_files/distress_refactored_pipeline_99_0.png)
...
def build_model_features_from_row(row: pd.Series) -> pd.DataFrame:
    ...
    log_mkvalt = np.log(mkvalt) if pd.notna(mkvalt) and mkvalt > 0 else np.nan
...
```
    ...
    Base PDs: {'pd_logit': 0.8285552917693227, 'pd_tree': 0.9577465057373047, 'pd_tree_raw': 0.9932275414466858}
